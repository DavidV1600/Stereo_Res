...
def parse_args():
    ...
    parser.add_argument('--model_name', type=str, default='PASSR_MHCA4_4xSR_iter1301')
    return parser.parse_args()

def test(test_loader, cfg):
    net = PASSRnet_with_MHCA(cfg.scale_factor).to(cfg.device)
    cudnn.benchmark = True
    pretrained_dict = torch.load('./log/' + '.pth.tar')
...
def main(cfg):
    test_set = TrainSetLoader(dataset_dir=cfg.testset_dir + '/' + cfg.dataset, scale_factor=cfg.scale_factor)
