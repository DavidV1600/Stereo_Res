...
class PASSRnet_with_MHCA(nn.Module):
    def __init__(self, upscale_factor):
        super(PASSRnet_with_MHCA, self).__init__()
        ### feature extraction
        ...
        self.mhca_buffer = MHCA(n_feats=64, ratio=2)
    ...
    def forward(self, x_left, x_right, is_training):
        ...
        if is_training == 1:
            ...
            buffer = self.mhca_buffer(buffer)
            ### upscaling
...
class PAM(nn.Module):
    def __init__(self, channels):
        ...
        self.mhca_buf = MHCA(n_feats=64, ratio=2)
...
if __name__ == "__main__":
    net = PASSRnet_with_MHCA(upscale_factor=4)
    total = sum([param.nelement() for param in net.parameters()])
    print('   Number of params: %.2fM' % (total / 1e6))

    pam = PAM(64)
    total = sum([param.nelement() for param in pam.parameters()])
    print('   Number of params: %.2fM' % (total / 1e6))

    mhca = MHCA(n_feats=64, ratio=2)
    total = sum([param.nelement() for param in mhca.parameters()])
    print('   Number of params: %.2fM' % (total / 1e6))