...
class SSRDEFNet_with_MHCA(nn.Module):
    def __init__(self, upscale_factor):
        super(SSRDEFNet_with_MHCA, self).__init__()
    ...
    def forward(self, x_left, x_right, is_training):
        ...
        disp1 = F.interpolate(disp1 * self.upscale_factor, scale_factor=self.upscale_factor, mode='bilinear',
                              align_corners=False)
        disp2 = F.interpolate(disp2 * self.upscale_factor, scale_factor=self.upscale_factor, mode='bilinear',
                              align_corners=False)
        ...
        disp_range_samples1 = get_disp_range_samples(cur_disp=disp1.detach().squeeze(1), ndisp=24,
                                                     shape=[x_left.shape[0], x_left.shape[2] * self.upscale_factor,
        ...
        buffer_leftF2 = self.CALayer2(
            err1 + feat_left)  # high resolution feature that contains high resolution information of the other image through high res stereo matching
        ...
        for ii in range(self.upscale_factor):
            for jj in range(self.upscale_factor):
                draft_l = dispwarpfeature(buffer_right, disp1_high[:, :, ii::self.upscale_factor,
                                                        jj::self.upscale_factor] / self.upscale_factor)
                draft_r = dispwarpfeature(buffer_left, disp2_high[:, :, ii::self.upscale_factor,
                                                       jj::self.upscale_factor] / self.upscale_factor)
        ...
        disp1_3 = F.interpolate(disp1_3 * self.upscale_factor, scale_factor=self.upscale_factor, mode='bilinear',
                                align_corners=False)
        disp2_3 = F.interpolate(disp2_3 * self.upscale_factor, scale_factor=self.upscale_factor, mode='bilinear',
                                align_corners=False)
        maxdisp = x_left.shape[3] * self.upscale_factor
        ...
        if is_training == 0:
            index = (torch.arange(w * self.upscale_factor).view(1, 1, w * self.upscale_factor).repeat(1,
            ...
            disp1, disp2), (disp1_3, disp2_3), (disp1_high, disp2_high), (disp1_high2, disp2_high2)
...
class Stereo_feature(nn.Module):
    def __init__(self):
        ...
        self.branch1 = nn.Sequential(nn.AvgPool2d((3, 3), stride=(3, 3)),  # Adjusted for 10x30 input
                                     convbn(64, 32, 1, 1, 0, 1),
                                     nn.PReLU())

        self.branch2 = nn.Sequential(nn.AvgPool2d((2, 2), stride=(2, 2)),  # Adjusted for 10x30 input
                                     convbn(64, 32, 1, 1, 0, 1),
                                     nn.PReLU())

        self.branch3 = nn.Sequential(nn.AvgPool2d((1, 1), stride=(1, 1)),  # Adjusted for 10x30 input
                                     convbn(64, 32, 1, 1, 0, 1),
                                     nn.PReLU())
...
class hourglass(nn.Module):
    def __init__(self, inplanes):
        ...
        self.conv5 = nn.Sequential(
            nn.ConvTranspose2d(inplanes, inplanes, kernel_size=3, padding=1, output_padding=1, stride=2, bias=False),
...
class hStereo_feature(nn.Module):
    def __init__(self):
        ...
        self.branch2 = nn.Sequential(nn.AvgPool2d((4, 4), stride=(4, 4)),  # Adjusted for 10x30 input
                                     convbn(64, 24, 1, 1, 0, 1),
                                     nn.PReLU())

        self.branch3 = nn.Sequential(nn.AvgPool2d((2, 2), stride=(2, 2)),  # Adjusted for 10x30 input
                                     convbn(64, 24, 1, 1, 0, 1),
                                     nn.PReLU())
...
class GetCostVolume(nn.Module):
    ...
    def forward(self, x, y, disp_range_samples1, disp_range_samples2, ndisp):
        ...
        for i in range(ndisp):
            cost1[:, i, :, :] = (Q * F.grid_sample(K, grid[:, i * height:(i + 1) * height, :, :], mode='bilinear',
                                                   padding_mode='zeros').view(bs, channels, height, width)).mean(dim=1)
        ...
        for i in range(ndisp):
            cost2[:, i, :, :] = (Q * F.grid_sample(K, grid[:, i * height:(i + 1) * height, :, :], mode='bilinear',
                                                   padding_mode='zeros').view(bs, channels, height, width)).mean(dim=1)
...
if __name__ == "__main__":
    net = SSRDEFNet_with_MHCA(upscale_factor=4)
