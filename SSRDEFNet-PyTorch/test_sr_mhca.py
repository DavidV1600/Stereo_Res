...
def test(cfg, loadname, net):
    ...
    for idx in range(len(file_list)):
        LR_left = Image.open(
            cfg.testset_dir + cfg.dataset + '/lr_x' + str(cfg.scale_factor) + '/' + file_list[idx] + '/lr0.png')
        LR_right = Image.open(
            cfg.testset_dir + cfg.dataset + '/lr_x' + str(cfg.scale_factor) + '/' + file_list[idx] + '/lr1.png')
        ...
        LR_left, LR_right, HR_left, HR_right = ToTensor()(LR_left), ToTensor()(LR_right), ToTensor()(
            HR_left), ToTensor()(HR_right)
        LR_left, LR_right, HR_left, HR_right = LR_left.unsqueeze(0), LR_right.unsqueeze(0), HR_left.unsqueeze(
            0), HR_right.unsqueeze(0)
        LR_left, LR_right, HR_left, HR_right = Variable(LR_left).cuda(), Variable(LR_right).cuda(), Variable(
            HR_left).cuda(), Variable(HR_right).cuda()
        scene_name = file_list[idx]
        # print('Running Scene ' + scene_name + ' of ' + cfg.dataset + ' Dataset......')
        with torch.no_grad():
            SR_left1, SR_right1, SR_left2, SR_right2, SR_left3, SR_right3, SR_left4, SR_right4, _, _, _, _ = net(
                LR_left, LR_right, is_training=0)
...
if __name__ == '__main__':
    ...
    # Flickr1024 da CUDA out of memory
    dataset_list = ['KITTI2012']
    net = SSRDEFNet_with_MHCA(cfg.scale_factor).cuda()
    net = torch.nn.DataParallel(net)
    net.eval()

    for j in range(81, 83):
        loadname = './checkpoints/SSRDEF_with_MHCA_4xSR_epoch' + str(j) + '.pth.tar'
        print(loadname)
