...
def parse_args():
    ...
    parser.add_argument('--n_epochs', type=int, default=83, help='number of epochs to train')
    parser.add_argument('--n_steps', type=int, default=10, help='number of epochs to update learning rate')
    parser.add_argument('--trainset_dir', type=str, default='./data/train/.20_60')
...
def warpfeature(feat, disp_range_samples, cost_prob, ndisp):
    ...
    mh, _ = torch.meshgrid([torch.arange(0, height, dtype=feat.dtype, device=feat.device),
                            torch.arange(0, width, dtype=feat.dtype, device=feat.device)])  # (H *W)
    ...
    grid = torch.stack([coords_x, coords_y], dim=4).view(bs, ndisp * height, width,
                                                         2)  # (B, D, H, W, 2)->(B, D*H, W, 2)
    ...
    warped_feat = cost_prob[:, 0, :, :].unsqueeze(1) * F.grid_sample(feat, grid[:, :height, :, :], mode='bilinear',
                                                                     padding_mode='zeros').view(bs, channels, height,
                                                                                                width)
    for i in range(1, ndisp):
        warped_feat += cost_prob[:, i, :, :].unsqueeze(1) * F.grid_sample(feat,
                                                                          grid[:, i * height:(i + 1) * height, :, :],
                                                                          mode='bilinear', padding_mode='zeros').view(
...
def train(train_loader, cfg):
    ...
    net = SSRDEFNet(cfg.scale_factor)
    ...
    psnr_epoch_m = []
    psnr_epoch_r_m = []
    print(cfg.start_epoch, cfg.n_epochs)
    for idx_epoch in range(cfg.start_epoch, cfg.n_epochs):
        print(f"EPOCH {idx_epoch} started...")
        for idx_iter, (HR_left, HR_right, LR_left, LR_right) in enumerate(train_loader):
            b, c, h, w = LR_left.shape
            _, _, h2, w2 = HR_left.shape
            HR_left, HR_right, LR_left, LR_right = Variable(HR_left), Variable(HR_right), \
                Variable(LR_left), Variable(LR_right)
            ...
            ''' SR Loss '''
            loss_SR = criterion_L1(SR_left, HR_left) + criterion_L1(SR_right, HR_right) + criterion_L1(SR_left2,
                                                                                                       HR_left) + criterion_L1(
                SR_right2, HR_right) + \
                      criterion_L1(SR_left3, HR_left) + criterion_L1(SR_right3, HR_right) + criterion_L1(SR_left4,
                                                                                                         HR_left) + criterion_L1(
                SR_right4, HR_right)
            ...
            #print(HR_left.shape, HR_right.shape)
            loss_P = loss_disp_unsupervised(HR_left, HR_right, disp1, F.interpolate(V_left, scale_factor=scale,
                                                                                    mode='nearest')) + loss_disp_unsupervised(
                HR_right, HR_left, disp2, F.interpolate(V_right, scale_factor=scale, mode='nearest')) + \
                     loss_disp_unsupervised(HR_left, HR_right, disp1_high, V_left2) + loss_disp_unsupervised(HR_right,
                                                                                                             HR_left,
                                                                                                             disp2_high,
                                                                                                             V_right2) + \
                     loss_disp_unsupervised(HR_left, HR_right, disp1_3, F.interpolate(V_left3, scale_factor=scale,
                                                                                      mode='nearest')) + loss_disp_unsupervised(
                HR_right, HR_left, disp2_3, F.interpolate(V_right3, scale_factor=scale, mode='nearest')) + \
                     loss_disp_unsupervised(HR_left, HR_right, disp1_high_2, V_left4) + loss_disp_unsupervised(HR_right,
                                                                                                               HR_left,
                                                                                                               disp2_high_2,
                                                                                                               V_right4)
            ...
            Res_leftT_low = torch.bmm(M_right_to_left.contiguous().view(b * h, w, w),
                                      Res_right_low.permute(0, 2, 3, 1).contiguous().view(b * h, w, c)
                                      ).view(b, h, w, c).contiguous().permute(0, 3, 1, 2)
            Res_rightT_low = torch.bmm(M_left_to_right.contiguous().view(b * h, w, w),
                                       Res_left_low.permute(0, 2, 3, 1).contiguous().view(b * h, w, c)
                                       ).view(b, h, w, c).contiguous().permute(0, 3, 1, 2)
            Res_leftT_low2 = torch.bmm(M_right_to_left3.contiguous().view(b * h, w, w),
                                       Res_right_low.permute(0, 2, 3, 1).contiguous().view(b * h, w, c)
                                       ).view(b, h, w, c).contiguous().permute(0, 3, 1, 2)
            Res_rightT_low2 = torch.bmm(M_left_to_right3.contiguous().view(b * h, w, w),
                                        Res_left_low.permute(0, 2, 3, 1).contiguous().view(b * h, w, c)
                                        ).view(b, h, w, c).contiguous().permute(0, 3, 1, 2)
            ...
            loss_photo = criterion_L1(Res_left_low * V_left.repeat(1, 3, 1, 1),
                                      Res_leftT_low * V_left.repeat(1, 3, 1, 1)) + \
                         criterion_L1(Res_right_low * V_right.repeat(1, 3, 1, 1),
                                      Res_rightT_low * V_right.repeat(1, 3, 1, 1)) + \
                         criterion_L1(Res_left * V_left2.repeat(1, 3, 1, 1), Res_leftT * V_left2.repeat(1, 3, 1, 1)) + \
                         criterion_L1(Res_right * V_right2.repeat(1, 3, 1, 1),
                                      Res_rightT * V_right2.repeat(1, 3, 1, 1)) + \
                         criterion_L1(Res_left_low * V_left3.repeat(1, 3, 1, 1),
                                      Res_leftT_low2 * V_left3.repeat(1, 3, 1, 1)) + \
                         criterion_L1(Res_right_low * V_right3.repeat(1, 3, 1, 1),
                                      Res_rightT_low2 * V_right3.repeat(1, 3, 1, 1)) + \
                         criterion_L1(Res_left * V_left4.repeat(1, 3, 1, 1), Res_leftT2 * V_left4.repeat(1, 3, 1, 1)) + \
                         criterion_L1(Res_right * V_right4.repeat(1, 3, 1, 1),
                                      Res_rightT2 * V_right4.repeat(1, 3, 1, 1))
            ...
            ''' Cycle Loss '''
            Res_left_cycle_low = torch.bmm(M_right_to_left.contiguous().view(b * h, w, w),
                                           Res_rightT_low.permute(0, 2, 3, 1).contiguous().view(b * h, w, c)
                                           ).view(b, h, w, c).contiguous().permute(0, 3, 1, 2)
            Res_right_cycle_low = torch.bmm(M_left_to_right.contiguous().view(b * h, w, w),
                                            Res_leftT_low.permute(0, 2, 3, 1).contiguous().view(b * h, w, c)
                                            ).view(b, h, w, c).contiguous().permute(0, 3, 1, 2)
            Res_left_cycle = dispwarpfeature(Res_rightT, disp1_high)
            Res_right_cycle = dispwarpfeature(Res_leftT, disp2_high)

            Res_left_cycle_low2 = torch.bmm(M_right_to_left3.contiguous().view(b * h, w, w),
                                            Res_rightT_low2.permute(0, 2, 3, 1).contiguous().view(b * h, w, c)
                                            ).view(b, h, w, c).contiguous().permute(0, 3, 1, 2)
            Res_right_cycle_low2 = torch.bmm(M_left_to_right3.contiguous().view(b * h, w, w),
                                             Res_leftT_low2.permute(0, 2, 3, 1).contiguous().view(b * h, w, c)
                                             ).view(b, h, w, c).contiguous().permute(0, 3, 1, 2)
            Res_left_cycle2 = dispwarpfeature(Res_rightT2, disp1_high_2)
            Res_right_cycle2 = dispwarpfeature(Res_leftT2, disp2_high_2)

            loss_cycle = criterion_L1(Res_left_low * V_left.repeat(1, 3, 1, 1),
                                      Res_left_cycle_low * V_left.repeat(1, 3, 1, 1)) + \
                         criterion_L1(Res_right_low * V_right.repeat(1, 3, 1, 1),
                                      Res_right_cycle_low * V_right.repeat(1, 3, 1, 1)) + \
                         criterion_L1(Res_left * V_left2.repeat(1, 3, 1, 1),
                                      Res_left_cycle * V_left2.repeat(1, 3, 1, 1)) + \
                         criterion_L1(Res_right * V_right2.repeat(1, 3, 1, 1),
                                      Res_right_cycle * V_right2.repeat(1, 3, 1, 1)) + \
                         criterion_L1(Res_left_low * V_left3.repeat(1, 3, 1, 1),
                                      Res_left_cycle_low2 * V_left3.repeat(1, 3, 1, 1)) + \
                         criterion_L1(Res_right_low * V_right3.repeat(1, 3, 1, 1),
                                      Res_right_cycle_low2 * V_right3.repeat(1, 3, 1, 1)) + \
                         criterion_L1(Res_left * V_left4.repeat(1, 3, 1, 1),
                                      Res_left_cycle2 * V_left4.repeat(1, 3, 1, 1)) + \
                         criterion_L1(Res_right * V_right4.repeat(1, 3, 1, 1),
                                      Res_right_cycle2 * V_right4.repeat(1, 3, 1, 1))
            ...
            SR_right_res = F.interpolate(torch.abs(HR_right - SR_right), scale_factor=1 / scale, mode='bicubic',
                                         align_corners=False)
            SR_left_res3 = F.interpolate(torch.abs(HR_left - SR_left3), scale_factor=1 / scale, mode='bicubic',
                                         align_corners=False)
            SR_right_res3 = F.interpolate(torch.abs(HR_right - SR_right3), scale_factor=1 / scale, mode='bicubic',
                                          align_corners=False)
            ...
            SR_left_resT3 = torch.bmm(M_right_to_left3.detach().contiguous().view(b * h, w, w),
                                      SR_right_res3.permute(0, 2, 3, 1).contiguous().view(b * h, w, c)
                                      ).view(b, h, w, c).contiguous().permute(0, 3, 1, 2)
            SR_right_resT3 = torch.bmm(M_left_to_right3.detach().contiguous().view(b * h, w, w),
                                       SR_left_res3.permute(0, 2, 3, 1).contiguous().view(b * h, w, c)
                                       ).view(b, h, w, c).contiguous().permute(0, 3, 1, 2)
            ...
            loss_cons = criterion_L1(SR_left_res * V_left.repeat(1, 3, 1, 1),
                        ...
                        criterion_L1(SR_left_res2 * V_left2.repeat(1, 3, 1, 1),
                                     SR_left_resT2 * V_left2.repeat(1, 3, 1, 1)) + \
                        criterion_L1(SR_right_res2 * V_right2.repeat(1, 3, 1, 1),
                                     SR_right_resT2 * V_right2.repeat(1, 3, 1, 1)) + \
                        criterion_L1(SR_left_res3 * V_left3.repeat(1, 3, 1, 1),
                                     SR_left_resT3 * V_left3.repeat(1, 3, 1, 1)) + \
                        criterion_L1(SR_right_res3 * V_right3.repeat(1, 3, 1, 1),
                                     SR_right_resT3 * V_right3.repeat(1, 3, 1, 1)) + \
                        criterion_L1(SR_left_res4 * V_left4.repeat(1, 3, 1, 1),
                                     SR_left_resT4 * V_left4.repeat(1, 3, 1, 1)) + \
                        criterion_L1(SR_right_res4 * V_right4.repeat(1, 3, 1, 1),
                                     SR_right_resT4 * V_right4.repeat(1, 3, 1, 1))
            ''' Total Loss '''
            loss = loss_SR + 0.1 * loss_cons + 0.1 * (
                        loss_photo + loss_smooth + loss_cycle) + 0.001 * loss_S + 0.01 * loss_P
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            #print("HERE", HR_left.shape, SR_left4.shape)
            psnr_epoch.append(cal_psnr(HR_left[:, :, :, 30:].data.cpu(), SR_left4[:, :, :, 30:].data.cpu()))
            psnr_epoch_r.append(cal_psnr(HR_right[:, :, :, :HR_right.shape[3] - 30].data.cpu(),
                                         SR_right4[:, :, :, :HR_right.shape[3] - 30].data.cpu()))

            psnr_epoch_m.append(cal_psnr(HR_left[:, :, :,30:].data.cpu(), SR_left2[:, :, :, 30:].data.cpu()))
